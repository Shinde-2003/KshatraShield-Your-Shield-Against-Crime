# KshatraShield – Your Shield Against Crime
An AI-powered crime reporting and analysis platform built with Python, Flask, MongoDB, NLP, JWT authentication, multilingual support, geolocation services, and real-time media evidence capture.

## Overview

KshatraShield is an AI-powered web-based crime reporting and analysis platform designed to modernize the process of crime reporting and improve communication between citizens and law enforcement agencies.

The platform enables users to report crimes using text, voice, images, videos, and audio evidence while ensuring privacy, security, and accessibility through multilingual support and anonymous reporting.


" USER REGISTRATION "

<img width="749" height="626" alt="image" src="https://github.com/user-attachments/assets/2a108b74-1db9-4310-a1b5-644d36eeee47" />


" USER LOGIN "

<img width="619" height="665" alt="image" src="https://github.com/user-attachments/assets/664ad3c7-51f6-4c2b-ad5c-fb9825bbeb9c" />


 " USER INTERFACE "

<img width="619" height="494" alt="image" src="https://github.com/user-attachments/assets/9fd5ed41-c4b1-4ad5-a2ce-5ba174d26f1b" />


 " CRIME ANALYSIS "

<img width="621" height="596" alt="image" src="https://github.com/user-attachments/assets/d7ce6cbc-cf52-4c46-a7c9-0103ded9a091" />


 " LIVE-CRIME CAPTURE "

<img width="515" height="671" alt="image" src="https://github.com/user-attachments/assets/6a542341-c24e-477f-8471-fb80a544cd00" />


 " OVERVIEW AND CRIME REPORTING PAGE " 

<img width="560" height="459" alt="image" src="https://github.com/user-attachments/assets/37ee8120-bc20-4b7d-9646-64c6d5dd5653" />


<img width="552" height="489" alt="image" src="https://github.com/user-attachments/assets/02ef5a42-72bb-49e5-84ac-1bb05ef9992c" />


 " DATABASE "

<img width="697" height="375" alt="image" src="https://github.com/user-attachments/assets/4094c6d2-be32-40ea-8d16-809218cb4ee8" />


<img width="670" height="350" alt="image" src="https://github.com/user-attachments/assets/3b7b5c67-6206-48e5-89b7-aa059fc7d67c" />


<img width="678" height="372" alt="image" src="https://github.com/user-attachments/assets/40204de1-a975-4e9e-b792-42f800ebce68" />


## Key Features

### Crime Reporting

* Online crime complaint registration
* Anonymous crime reporting
* Voice-based complaint submission
* Text-based complaint submission
* Image, video, and audio evidence upload
* Real-time media capture

### Security & Authentication

* JWT Authentication
* Password Hashing using Bcrypt
* Secure User Verification
* Data Encryption
* Privacy Protection

### AI & NLP Integration

* Automatic Crime Classification
* Keyword-Based NLP Analysis
* Crime Categorization
* Crime Priority Prediction
* Incident Severity Assessment

### Location Services

* Geolocation API Integration
* Automatic Location Detection
* Crime Mapping Support

### Dashboard & Analytics

* Crime Statistics Dashboard
* Category Wise Analysis
* Priority Level Visualization
* Interactive Charts and Graphs
* Trend Analysis

### Accessibility

* English Language Support
* Hindi Language Support
* Marathi Language Support

## Technologies Used

### Frontend

* HTML5
* CSS3
* JavaScript

### Backend

* Python
* Flask

### Database

* MongoDB

### Security

* JWT Authentication
* Bcrypt Password Hashing

### APIs

* Geolocation API
* Media Capture API
* Fetch API

### Visualization

* Chart.js

### AI / NLP

* Keyword Based NLP Classification
* Crime Priority Analysis

## System Workflow

1. User Registration/Login
2. Crime Reporting
3. Evidence Upload
4. NLP Crime Classification
5. Priority Assignment
6. Database Storage
7. Dashboard Analysis
8. Complaint Tracking

## Project Objectives

* Simplify crime reporting
* Improve public safety
* Enable anonymous reporting
* Enhance law enforcement response
* Provide crime analytics
* Improve transparency
* Bridge the gap between citizens and police

## Skills Demonstrated

* Full Stack Web Development
* Cyber Security
* MongoDB Database Management
* REST API Development
* Authentication & Authorization
* Natural Language Processing
* Data Visualization
* Crime Analytics
* Geolocation Integration
* Secure Software Development

## Future Enhancements

* AI Chatbot Integration
* Face Recognition Support
* Predictive Crime Analytics
* Mobile Application
* Real-Time Police Integration
* Emergency SOS Features
* Advanced Machine Learning Models

## Author
Rushi Prakash Shinde

